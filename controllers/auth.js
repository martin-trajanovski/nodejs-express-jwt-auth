...
let registerUser = (firstName, lastName, email, password) => {
...
let createToken = user => {
  return jwt.sign(_.omit(user.toObject(), 'password'), config.authTokenSecret, {
    expiresIn: '1h' // lower value for testing
  });
};

let createRefreshToken = user => {
  let refreshToken = jwt.sign({ type: 'refresh' }, config.refreshTokenSecret, {
    expiresIn: '1d' // 1 day
  });
